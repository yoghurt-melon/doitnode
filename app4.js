...
function connectDB() {
    ...
    database.on('open', function () {
        ...
        /*
        UserSchemes.statics.findById = function (id, callback) {
            return this.find({ id: id }, callback);
        }
        */
...
router.route('/listuser').post(function (req, res) {
    console.log('/listuser 라우팅 함수 호출됨.');
    ...
    if (database) {
        UserModel.findAll(function (err, results) {
            ...
            if (results) {
                ...
                res.write('<div><ul>');
                results.forEach((e, i) => {
                    const curId = e._doc.id;
                    const curName = e._doc.name;
    ...
    UserModel.findById(id, function (err, results) {
        ...
        if (results.length > 0) {
            if (results[0]._doc.password === password) {
                ...
                console.log('비밀번호 일치하치 않음');
                callback(null, null);
            }
        } else {
            console.log('해당 아이디에 해당하는 사용자 없음');
            callback(null, null);
        }
    });
